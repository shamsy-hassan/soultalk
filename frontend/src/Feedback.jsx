  ...
  return (
    <div className="space-y-4">
      <div className="surface-panel p-5">
        <h1 className="text-2xl font-bold tracking-tight text-soultalk-dark-gray">{t('send_feedback')}</h1>
        <p className="text-sm text-soultalk-medium-gray mt-1">{t('feedback_subtitle')}</p>
      </div>

      <form onSubmit={onSubmit} className="card p-5 rounded-2xl space-y-3">
        ...
        {status && <p className="text-sm text-soultalk-medium-gray">{status}</p>}
      </form>
    </div>
  );
};
